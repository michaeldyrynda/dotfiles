...
- **Group implementation details by user-visible behaviour, not by technical layer.** When documenting changes to existing features, organise sections around what the user sees differently (e.g. "Broker selects relationship duration", "Broker enters business banking details") rather than by layer (e.g. "Validation Rules", "DTO Updates", "Frontend Changes"). Each behavioural group should describe the change across all layers it touches. This prevents the task planner from decomposing horizontally along the spec's own section boundaries. Pure technical sections (Enums, Config, Edge Cases) remain as-is — the guidance applies to sections that describe feature changes.
